...
public:
    void Begin(benchmark::State& state)
    {
        if (ThreadCount_++ == 0) {
            Items_ = 0;
            ThreadPool_ = New<TThreadPool>(state.range(0), "Bench");
        }
    }
    ...
    int GetItems()
    {
        int i = Items_.exchange(-1000000, std::memory_order_relaxed);
        return i > 0 ? i : 0;
    }
...
{
    static TThreadPoolPtr ThreadPool;

    if (state.thread_index == 0) {
        // Add extra sleeping threads.
        ThreadPool = New<TThreadPool>(state.threads + state.range(0), "Scaling");
    }
