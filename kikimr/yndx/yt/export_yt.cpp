...
namespace NCustomErrorCodes {
    ...
    constexpr int InvalidNodeType = 100000;
...
class TYtUploader: public TActorBootstrapped<TYtUploader> {
    ...
    static TVector<TColumnSchema> GenColumnsSchema(const TTableColumns& columns) {
        ...
        for (const auto& [_, column] : columns) {
            schema.emplace_back(column.Name, ConvertType(column.Type));
        }
    ...
    static bool IsRetryableError(const NYT::TError& error) {
        if (// Table errors
            ...
            || error.FindMatching(NYT::NTableClient::EErrorCode::IncomparableType)
    ...
    void Handle(TEvYtWrapper::TEvCreateNodeResponse::TPtr& ev) {
        ...
        Send(Client, new TEvYtWrapper::TEvCreateTableWriterRequest(DstPath, TTableWriterOptions()));
    }
    ...
    void RetryOrFinish(const NYT::TError& error) {
        if (Attempt++ < Retries && IsRetryableError(error)) {
            ...
            Finish(false, TStringBuilder() << "YT error: " << error.GetMessage());
        }
    }
...
public:
    ...
    void Collect(const NTable::IScan::TRow& row) override {
        ...
        for (ui32 i = 0; i < (*row).size(); ++i) {
            Y_VERIFY(i < OrderedColumns.size());
...
IBuffer* TYtExport::CreateBuffer(ui64 rowsLimit, ui64 bytesLimit) const {
    const auto& settings = Task.GetYTSettings();
    return new TYtBuffer(Columns, rowsLimit, bytesLimit, settings.GetUseTypeV3());
}
