...
public:
            ...
            TAutoPtr<TLogBackend> logBackend = {})
    {
        ...
        ServerSettings->RegisterGrpcService<NKikimr::NGRpcService::TYndxKeyValueGRpcService>("keyvalue");
...
Y_UNIT_TEST_SUITE(KeyValueGRPCService) {
    ...
    void MakeDirectory(auto &channel, const TString &path) {
        ...
        UNIT_ASSERT_CHECK_STATUS(makeDirectoryResponse.operation().status(), Ydb::StatusIds::SUCCESS);
    }

    void MakeTable(auto &channel, const TString &path) {
        std::unique_ptr<Ydb::Yndx::KeyValue::V1::KeyValueService::Stub> stub;
        stub = Ydb::Yndx::KeyValue::V1::KeyValueService::NewStub(channel);

        Ydb::Yndx::KeyValue::CreateVolumeRequest createVolumeRequest;
        createVolumeRequest.set_path(path);
        createVolumeRequest.set_storage_channel_profile_id(1);
        createVolumeRequest.set_partition_count(1);

        Ydb::Yndx::KeyValue::CreateVolumeResponse createVolumeResponse;
        Ydb::Yndx::KeyValue::CreateVolumeResult createVolumeResult;
        ...
        UNIT_ASSERT_CHECK_STATUS(createVolumeResponse.operation().status(), Ydb::StatusIds::SUCCESS);
    ...
    void DropVolume(auto &channel, const TString &path) {
        ...
        Ydb::Yndx::KeyValue::DropVolumeRequest dropVolumeRequest;
        dropVolumeRequest.set_path(path);

        Ydb::Yndx::KeyValue::DropVolumeResponse dropVolumeResponse;
        Ydb::Yndx::KeyValue::DropVolumeResult dropVolumeResult;
        ...
        UNIT_ASSERT_CHECK_STATUS(dropVolumeResponse.operation().status(), Ydb::StatusIds::SUCCESS);
    ...
    Ydb::Scheme::ListDirectoryResult ListDirectory(auto &channel, const TString &path) {
        ...
        UNIT_ASSERT_CHECK_STATUS(listDirectoryResponse.operation().status(), Ydb::StatusIds::SUCCESS);
    ...
    void MakeSimpleTest(const TString &tablePath,
            std::function<void(const std::unique_ptr<Ydb::Yndx::KeyValue::V1::KeyValueService::Stub>&)> func)
    {
    ...
    Y_UNIT_TEST(SimpleAcquireLock) {
        ...
        MakeSimpleTest(tablePath, [tablePath](const std::unique_ptr<Ydb::Yndx::KeyValue::V1::KeyValueService::Stub> &stub){
            Ydb::Yndx::KeyValue::AcquireLockRequest request;
            request.set_path(tablePath);
            request.set_partition_id(0);
            Ydb::Yndx::KeyValue::AcquireLockResponse response;
            Ydb::Yndx::KeyValue::AcquireLockResult result;

            grpc::ClientContext ctx1;
            AdjustCtxForDB(ctx1);
            stub->AcquireLock(&ctx1, request, &response);
            UNIT_ASSERT_CHECK_STATUS(response.operation().status(), Ydb::StatusIds::SUCCESS);
    ...
    Y_UNIT_TEST(SimpleExecuteTransaction) {
        TString tablePath = "/Root/mydb/kvtable";
        MakeSimpleTest(tablePath, [tablePath](const std::unique_ptr<Ydb::Yndx::KeyValue::V1::KeyValueService::Stub> &stub){
            Ydb::Yndx::KeyValue::ExecuteTransactionRequest request;
            request.set_path(tablePath);
            request.set_partition_id(0);
            Ydb::Yndx::KeyValue::ExecuteTransactionResponse response;
    ...
    Y_UNIT_TEST(SimpleExecuteTransactionWithWrongGeneration) {
        ...
        MakeSimpleTest(tablePath, [tablePath](const std::unique_ptr<Ydb::Yndx::KeyValue::V1::KeyValueService::Stub> &stub){
            ...
            AdjustCtxForDB(ctx);
            stub->ExecuteTransaction(&ctx, request, &response);
            UNIT_ASSERT_CHECK_STATUS(response.operation().status(), Ydb::StatusIds::PRECONDITION_FAILED);
        });
    }

    void Write(const TString &path, ui64 partitionId, const TString &key, const TString &value, ui64 storageChannel,
            const std::unique_ptr<Ydb::Yndx::KeyValue::V1::KeyValueService::Stub> &stub)
    {
        Ydb::Yndx::KeyValue::ExecuteTransactionRequest writeRequest;
        ...
        Ydb::Yndx::KeyValue::ExecuteTransactionResponse writeResponse;

        grpc::ClientContext writeCtx;
        AdjustCtxForDB(writeCtx);
        stub->ExecuteTransaction(&writeCtx, writeRequest, &writeResponse);
        UNIT_ASSERT_CHECK_STATUS(writeResponse.operation().status(), Ydb::StatusIds::SUCCESS);
    }
    ...
    Y_UNIT_TEST(SimpleWriteRead) {
        ...
        MakeSimpleTest(tablePath, [tablePath](const std::unique_ptr<Ydb::Yndx::KeyValue::V1::KeyValueService::Stub> &stub){
            ...
            Ydb::Yndx::KeyValue::ReadRequest readRequest;
            readRequest.set_path(tablePath);
            readRequest.set_partition_id(0);
            readRequest.set_key("key");
            Ydb::Yndx::KeyValue::ReadResponse readResponse;
            Ydb::Yndx::KeyValue::ReadResult readResult;

            grpc::ClientContext readCtx;
            AdjustCtxForDB(readCtx);
            stub->Read(&readCtx, readRequest, &readResponse);
            UNIT_ASSERT_CHECK_STATUS(readResponse.operation().status(), Ydb::StatusIds::SUCCESS);
    ...
    Y_UNIT_TEST(SimpleWriteReadRange) {
        ...
        MakeSimpleTest(tablePath, [tablePath](const std::unique_ptr<Ydb::Yndx::KeyValue::V1::KeyValueService::Stub> &stub){
            ...
            Ydb::Yndx::KeyValue::ReadRangeRequest readRangeRequest;
            ...
            Ydb::Yndx::KeyValue::ReadRangeResponse readRangeResponse;
            Ydb::Yndx::KeyValue::ReadRangeResult readRangeResult;

            grpc::ClientContext readRangeCtx;
            AdjustCtxForDB(readRangeCtx);
            stub->ReadRange(&readRangeCtx, readRangeRequest, &readRangeResponse);
            UNIT_ASSERT_CHECK_STATUS(readRangeResponse.operation().status(), Ydb::StatusIds::SUCCESS);
    ...
    Y_UNIT_TEST(SimpleWriteListRange) {
        ...
        MakeSimpleTest(tablePath, [tablePath](const std::unique_ptr<Ydb::Yndx::KeyValue::V1::KeyValueService::Stub> &stub){
            ...
            Ydb::Yndx::KeyValue::ListRangeRequest listRangeRequest;
            listRangeRequest.set_path(tablePath);
            listRangeRequest.set_partition_id(0);
            auto *r = listRangeRequest.mutable_range();
            r->set_from_key_inclusive("key1");
            r->set_to_key_inclusive("key3");
            Ydb::Yndx::KeyValue::ListRangeResponse listRangeResponse;
            Ydb::Yndx::KeyValue::ListRangeResult listRangeResult;

            grpc::ClientContext listRangeCtx;
            AdjustCtxForDB(listRangeCtx);
            stub->ListRange(&listRangeCtx, listRangeRequest, &listRangeResponse);
            UNIT_ASSERT_CHECK_STATUS(listRangeResponse.operation().status(), Ydb::StatusIds::SUCCESS);
    ...
    Y_UNIT_TEST(SimpleGetStorageChannelStatus) {
        TString tablePath = "/Root/mydb/kvtable";
        MakeSimpleTest(tablePath, [tablePath](const std::unique_ptr<Ydb::Yndx::KeyValue::V1::KeyValueService::Stub> &stub){
            Ydb::Yndx::KeyValue::GetStorageChannelStatusRequest getStatusRequest;
            ...
            Ydb::Yndx::KeyValue::GetStorageChannelStatusResponse getStatusResponse;
            Ydb::Yndx::KeyValue::GetStorageChannelStatusResult getStatusResult;
            ...
            UNIT_ASSERT_CHECK_STATUS(getStatusResponse.operation().status(), Ydb::StatusIds::SUCCESS);
    ...
    Y_UNIT_TEST(SimpleCreateAndDropVolume) {
        TKikimrWithGrpcAndRootSchema server;
        ui16 grpc = server.GetPort();
        TString location = TStringBuilder() << "localhost:" << grpc;
        ...
        auto *result = server.GetRuntime()->GrabEdgeEvent<TEvTxProxySchemeCache::TEvNavigateKeySetResult>(handle);
        UNIT_ASSERT_VALUES_EQUAL(result->Request->ResultSet.size(), 1);
        UNIT_ASSERT(result->Request->ResultSet[0].SolomonVolumeInfo);
    ...
    Y_UNIT_TEST(SimpleListPartitions) {
        ...
        Ydb::Yndx::KeyValue::ListLocalPartitionsRequest enumerateRequest;
        enumerateRequest.set_path(tablePath);
        enumerateRequest.set_node_id(2);

        Ydb::Yndx::KeyValue::ListLocalPartitionsResult enumerateResult;
        Ydb::Yndx::KeyValue::ListLocalPartitionsResponse eumerateResponse;
        ...
        UNIT_ASSERT_CHECK_STATUS(eumerateResponse.operation().status(), Ydb::StatusIds::SUCCESS);
