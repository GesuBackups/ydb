...
//        server.AnnoyingClient->ModifyACL("/Root/account1", "root-acc-topic", acl.SerializeAsString());
    ...
    Y_UNIT_TEST(PartitionsMapping) {
        ...
        TTestPQLib pqLib(server);
        auto consumerSettings = pqLib.MakeSettings({shortTopicName});
        consumerSettings.UseLockSession = true;
        consumerSettings.PartitionGroups = {2, 4};
//        for (auto i = 1u; i <= partsCount; i++) {
//            consumerSettings.PartitionGroups.push_back(i);
//        }
        auto[consumer, response] = pqLib.CreateConsumer(consumerSettings);
        UNIT_ASSERT_C(response.Response.HasInit(), response.Response);
        THashSet<ui32> locksGot = {};
        while(locksGot.size() < consumerSettings.PartitionGroups.size()) {
            auto msg = consumer->GetNextMessage().GetValue(TDuration::Seconds(10));
            //Cerr << "Got message: " << msg.Response.ShortDebugString() << Endl;
            UNIT_ASSERT(msg.Type == EMessageType::EMT_LOCK);
            UNIT_ASSERT_VALUES_EQUAL(msg.Response.GetLock().GetTopic(), topicName);
            auto partId = msg.Response.GetLock().GetPartition();
            UNIT_ASSERT(!locksGot.contains(partId));
            UNIT_ASSERT(partId == 1 || partId == 3);
            locksGot.insert(partId);
        }
        UNIT_ASSERT_VALUES_EQUAL(locksGot.size(), 2);
        auto[consumer2, response2] = pqLib.CreateConsumer(consumerSettings);
        UNIT_ASSERT_C(response2.Response.HasInit(), response2.Response);
        THashSet<ui32> releasesGot = {};
        while(releasesGot.size() == 0) {
            auto msg = consumer->GetNextMessage().GetValue(TDuration::Seconds(10));
            Cerr << "Got message: " << msg.Response.ShortDebugString() << Endl;
            UNIT_ASSERT(msg.Type == EMessageType::EMT_RELEASE);
            UNIT_ASSERT_VALUES_EQUAL(msg.Response.GetRelease().GetTopic(), topicName);
            auto partId = msg.Response.GetRelease().GetPartition();
