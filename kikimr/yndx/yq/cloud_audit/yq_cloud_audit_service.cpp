...
#include <cloud/bitbucket/public-api/yandex/cloud/events/yq/yq.pb.h>
...
void FillAuthentication(::yandex::cloud::events::Authentication& authentication, const NYq::TEvAuditService::TExtraInfo& info) {
    ...
    authentication.set_subject_id(info.User);
...
void FillResponse(TEvent& cloudEvent, const NYql::TIssues& issues) {
    ...
    if (issues.Empty()) {
        ...
        error->set_code(grpc::StatusCode::OK);
...
public:
    ...
    void Bootstrap(const NActors::TActorContext&) {
        ...
        Base::Send(NKikimr::NFolderService::FolderServiceActorId(), request.release(), 0, 0);
    }
...
public:
    ...
    TYqCloudAuditServiceActor(const NConfig::TAuditConfig& config)
...
NActors::IActor* CreateYqCloudAuditServiceActor(const NConfig::TAuditConfig& config) {
    return new TYqCloudAuditServiceActor(config);
}
