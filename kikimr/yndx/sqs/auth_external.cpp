...
public:
    ...
    void Handle(TEvListFolderRequest::TPtr& ev) {
        const auto folder = ev->Get()->Request.folder_id();
        THolder<TEvListFolderResponse> result = MakeHolder<TEvListFolderResponse>();

        if (++RequestNumber % 3 == 0) {
            result->Status = NGrpc::TGrpcStatus("Shit happened", grpc::StatusCode::INTERNAL, false);
        } else {
            if (folder != "FOLDER_alkonavt") {
                auto* res = result->Response.mutable_folder();
                res->Setid(folder);
                res->Setcloud_id(TString("CLOUD_FOR_") + folder);
                result->Status = NGrpc::TGrpcStatus("OK", grpc::StatusCode::OK, false);
            } else {
                result->Status = NGrpc::TGrpcStatus("Auth error", grpc::StatusCode::UNAUTHENTICATED, false);
            }
