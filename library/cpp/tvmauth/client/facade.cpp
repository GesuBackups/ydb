...
namespace NTvmAuth {
        ...
        : Updater_(NTvmApi::TThreadedUpdater::Create(settings, std::move(logger)))
    {
        ...
        if (settings.CheckServiceTickets) {
            Service_ = MakeHolder<TServiceTicketChecker>(Updater_);
        }
        if (settings.CheckUserTicketsWithBbEnv) {
            User_ = MakeHolder<TUserTicketChecker>(Updater_);
        }
        if (settings.FetchRolesForIdmSystemSlug && settings.ShouldCheckSrc) {
            SrcChecker_ = MakeHolder<TSrcChecker>(Updater_);
        }
        if (settings.FetchRolesForIdmSystemSlug && settings.ShouldCheckDefaultUid) {
